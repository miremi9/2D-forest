...
class Tree:
        ...
        def __init__(self,angle:tuple,pos:tuple,root=None,size:int=100,time2live:int=10,
                          ...
                          chance_of_branch:float=1,chance_of_leave:float=0.5):
        ...
        def grow_branch(self):
                ...
                for a in li[:self.nb_branch]:
                        color = self.get_variation_color_branch()
                        tick = self.get_variation_tick_branch()
                        time2live = self.get_varation_time2live_branch()
                        scope = self.get_varation_scope_branch()
                        scale = self.get_variation_scale_branch()
                        chance_branch = self.get_variation_chance_of_branch_branch()
                        chance_sub = self.get_variation_chance_of_sub_branch()
                        t1 = Tree(a,self.head.pos,self,self.size*scale,time2live,scope,color,self.absolute_scope,tick,chance_branch,chance_sub)
                        self.trees.add(t1)
        ...
        def grow_sub_tree(self,branch:Branch):
                ...
                color = self.get_variation_color_sub()
                tick = self.get_variation_tick_sub()
                time2live = self.get_varation_time2live_sub()
                scope = self.get_varation_scope_sub()
                scale = self.get_varation_scale_sub()
                chance_branch = self.get_varation_chance_of_branch_sub()
                chance_sub = self.get_varation_chance_of_sub_sub()
                mid = branch.get_mid()
                nb_sub = self.nb_branch
                li = [-45,45]
                shuffle(li)
                for iangle in li[:nb_sub]:
                        angle = self.angle+iangle
                        
                        t1 = Tree(angle,mid,self,self.size*scale,time2live,scope,color,self.absolute_scope,tick,chance_branch,chance_sub)
                        self.trees.add(t1)
        ...
        def get_variation_color_branch(self):
                return generate_color(self.color,variation=10)
        
        def get_variation_scale_branch(self):
                return 0.90
        
        def get_variation_tick_branch(self):

                return 0.1+10/self.size

        def get_variation_chance_of_branch_branch(self):
                return self.chance_of_branch-0.25

        def get_variation_chance_of_sub_branch(self):
                return  self.chance_of_leave

        def get_varation_time2live_branch(self):
                if tools.distance(self.get_seed(),self.head.pos) > 1000:
                        return 0
                else:
                        return  average(self.time2live-self.timelived,10)
                return self.time2live/1.2
        
        def get_varation_scope_branch(self):
                return self.scope
        
        def get_varation_scale_branch(self):
                return 0.5

        #--------------------------        
        def get_variation_color_sub(self):
                return generate_color(self.color,variation=10)
        
        def get_variation_tick_sub(self):
                return self.tick *2
                return 0.1+10/self.size
        
        def get_varation_time2live_sub(self):
                return self.get_varation_time2live_branch()
        
        def get_varation_scope_sub(self):
                return self.scope
        
        def get_varation_scale_sub(self):
                return 0.5

        def get_varation_chance_of_sub_sub(self):
                return self.chance_of_leave-0.25

        def get_varation_chance_of_branch_sub(self):
                return self.chance_of_branch
class Leave:
